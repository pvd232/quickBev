...
public class STPSource: NSObject, STPAPIResponseDecodable, STPSourceProtocol, STPPaymentOption {
    ...
    class func flow(from string: String) -> STPSourceFlow {
        let key = string.lowercased()
        let flowNumber = stringToFlowMapping()[key]
    ...
    class func string(from flow: STPSourceFlow) -> String? {
        return
            (stringToFlowMapping() as NSDictionary).allKeys(for: NSNumber(value: flow.rawValue))
                .first as? String
    }
    ...
    class func status(from string: String) -> STPSourceStatus {
        let key = string.lowercased()
        let statusNumber = stringToStatusMapping()[key]
    ...
    class func string(from status: STPSourceStatus) -> String? {
        return
            (stringToStatusMapping() as NSDictionary).allKeys(for: NSNumber(value: status.rawValue))
                .first as? String
    }
    ...
    class func usage(from string: String) -> STPSourceUsage {
        let key = string.lowercased()
        let usageNumber = stringToUsageMapping()[key]
    ...
    class func string(from usage: STPSourceUsage) -> String? {
        return
            (stringToUsageMapping() as NSDictionary).allKeys(for: NSNumber(value: usage.rawValue))
                .first as? String
    }
    ...
    public class func decodedObject(fromAPIResponse response: [AnyHashable: Any]?) -> Self? {
        ...
        source.status = status(from: rawStatus ?? "")
        source.type = type(from: rawType ?? "")
        let rawUsage = dict.stp_string(forKey: "usage")
        source.usage = usage(from: rawUsage ?? "")
