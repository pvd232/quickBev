...
public class STPPaymentIntentParams: NSObject {
    ...
    internal static let isClientSecretValidRegex: NSRegularExpression? = try? NSRegularExpression(
        pattern: "^pi_[^_]+_secret_[^_]+$", options: []
    )

    internal class func isClientSecretValid(_ clientSecret: String) -> Bool {
        return
