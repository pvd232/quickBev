...
class STPPaymentOptionTableViewCell: UITableViewCell {
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        NSLayoutConstraint.activate(
            [
                leftIcon.centerXAnchor.constraint(
                    equalTo: contentView.leadingAnchor, constant: kPadding + 0.5 * kDefaultIconWidth
                ),
                leftIcon.centerYAnchor.constraint(lessThanOrEqualTo: contentView.centerYAnchor),
                checkmarkIcon.widthAnchor.constraint(equalToConstant: kCheckmarkWidth),
                checkmarkIcon.heightAnchor.constraint(
                    equalTo: checkmarkIcon.widthAnchor, multiplier: 1.0
                ),
                checkmarkIcon.centerXAnchor.constraint(
                ...
                titleLabel.trailingAnchor.constraint(
                    equalTo: checkmarkIcon.leadingAnchor, constant: -kPadding
                ),
        ...
        let fadedColor: UIColor = {
            if #available(iOS 13.0, *) {
                return UIColor(dynamicProvider: { _ in
                    self.theme.primaryForegroundColor.withAlphaComponent(0.6)
                })
            } else {
    ...
    func buildAttributedString(
        ...
        let secondaryColor: UIColor = {
            if #available(iOS 13.0, *) {
                return UIColor(dynamicProvider: { _ in
                    primaryColor.withAlphaComponent(0.6)
                })
            } else {
