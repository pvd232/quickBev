...
public extension STPAPIClient {
    ...
    class func pkPaymentError(forStripeError stripeError: Error?) -> Error? {
    ...
    internal class func billingDetails(from payment: PKPayment) -> STPPaymentMethodBillingDetails? {
    ...
    internal class func addressParams(from contact: PKContact?) -> [AnyHashable: Any]? {
    ...
    internal class func parameters(for payment: PKPayment) -> [String: Any] {
        let paymentString = String(data: payment.token.paymentData, encoding: .utf8)
        var payload: [String: Any] = [:]
        payload["pk_token"] = paymentString
        if let billingContact = payment.billingContact {
            payload["card"] = addressParams(from: billingContact)
        }
