...
class STPPostalCodeValidator: NSObject {
    class func postalCodeIsRequired(forCountryCode countryCode: String?) -> Bool {
        if countryCode == nil {
            ...
            return
                !(countriesWithNoPostalCodes()?.contains(countryCode?.uppercased() ?? "") ?? false)
        }
    }
    ...
    class func validationState(
        ...
        if postalCodeIsRequired(forCountryCode: countryCode) {
            if sanitizedCountryCode == STPCountryCodeUnitedStates {
                return validationState(forUSPostalCode: postalCode)
            } else {
    ...
    class func formattedSanitizedPostalCode(
        ...
        if usage != .cardField, sanitizedCountryCode == STPCountryCodeUnitedStates {
            return formattedSanitizedUSZipCode(
                from: postalCode,
                usage: usage
            )
        } else {
            return formattedSanitizedPostalCode(from: postalCode)
        }
    }
