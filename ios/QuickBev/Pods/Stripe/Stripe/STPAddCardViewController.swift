    ...
    UITableViewDataSource
{
    ...
    /// :nodoc:
    @objc
    override public func handleCancelTapped(_: Any?) {
        delegate?.addCardViewControllerDidCancel(self)
    }

    @objc func nextPressed(_: Any?) {
        loading = true
    ...
    @objc
    public func paymentCardTextFieldWillEndEditing(forReturn _: STPPaymentCardTextField) {
    ...
    @objc
    public func paymentCardTextFieldDidBeginEditing(_: STPPaymentCardTextField) {
        if #available(iOS 13.0, *) {
            cardScanner?.stop()
        }
    }
    ...
    func addressViewModel(_: STPAddressViewModel, addedCellAt index: Int) {
        let indexPath = IndexPath(
    ...
    func addressViewModel(_: STPAddressViewModel, removedCellAt index: Int) {
        let indexPath = IndexPath(
    ...
    func addressViewModelDidChange(_: STPAddressViewModel) {
        updateDoneButton()
    }

    func addressViewModelWillUpdate(_: STPAddressViewModel) {
        tableView?.beginUpdates()
    }

    func addressViewModelDidUpdate(_: STPAddressViewModel) {
        tableView?.endUpdates()
    }
    ...
    /// :nodoc:
    @objc
    public func tableView(_: UITableView, viewForFooterInSection _: Int)
        -> UIView?
    {
        return UIView()
    }

    @objc func useShippingAddress(_: UIButton) {
        tableView?.beginUpdates()
    ...
    @available(iOS 13, *)
    func cardScanner(
        _: STPCardScanner, didFinishWith cardParams: STPPaymentMethodCardParams?, error: Error?
    ) {
        if let error = error {
            handleError(error)
        }
