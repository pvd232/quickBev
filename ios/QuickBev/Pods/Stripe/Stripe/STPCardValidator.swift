...
public class STPCardValidator: NSObject {
    ...
    public class func validationState(
        ...
        let sanitizedNumber = stringByRemovingSpaces(from: cardNumber)
        if sanitizedNumber.count == 0 {
            return .incomplete
        }
        if !stringIsNumeric(sanitizedNumber) {
            return .invalid
        }
        ...
        if sanitizedNumber.count == binRange.length {
            let isValidLuhn = stringIsValidLuhn(sanitizedNumber)
            if isValidLuhn {
    ...
    public class func brand(forNumber cardNumber: String) -> STPCardBrand {
        let sanitizedNumber = sanitizedNumericString(for: cardNumber)
        let brands = possibleBrands(forNumber: sanitizedNumber)
        if brands.count == 1 {
            return brands.first!
        }
        return .unknown
    ...
    public class func maxLength(for brand: STPCardBrand) -> Int {
        var maxLength = -1
        for length in lengths(for: brand) {
            if length > maxLength {
    ...
    public class func fragmentLength(for brand: STPCardBrand) -> Int {
        return Int(cardNumberFormat(for: brand).last?.uintValue ?? 0)
    }
    ...
    public class func validationState(forExpirationMonth expirationMonth: String)
        -> STPCardValidationState
    {
        let sanitizedExpiration = stringByRemovingSpaces(from: expirationMonth)

        if !stringIsNumeric(sanitizedExpiration) {
            return .invalid
        }
        ...
        case 2:
            return (Int(sanitizedExpiration) ?? 0 > 0 && Int(sanitizedExpiration) ?? 0 <= 12)
                ? .valid : .invalid
        default:
            return .invalid
        }
    ...
    public class func validationState(forCVC cvc: String, cardBrand brand: STPCardBrand)
        -> STPCardValidationState
    {
        if !stringIsNumeric(cvc) {
            return .invalid
        }

        let sanitizedCvc = sanitizedNumericString(for: cvc)

        let minLength = minCVCLength()
        let maxLength = maxCVCLength(for: brand)
    ...
    class func validationState(
        ...
        if !stringIsNumeric(expirationMonth) || !stringIsNumeric(expirationYear) {
            return .invalid
        }

        let sanitizedMonth = sanitizedNumericString(for: expirationMonth)
        let sanitizedYear = sanitizedNumericString(for: expirationYear)
    ...
    class func validationState(
        ...
        let cvcValidation = validationState(forCVC: card.cvc ?? "", cardBrand: brand)
...
extension STPCardValidator {
    ...
    class func cardNumberFormat(forCardNumber cardNumber: String) -> [NSNumber] {
        ...
        return cardNumberFormat(for: binRange.brand)
    }
